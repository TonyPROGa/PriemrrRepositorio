...
def apertura_caja_view(request):
    return render(request, "apert_caja.html")

def apert_caja(request):
    monto_apertura = None

    if request.method == 'POST':
        monto_apertura = request.POST.get('monto')

    return render(request, 'apert_caja.html', {'monto_apertura': monto_apertura})
